...
export async function buildDashboard(): Promise<Dashboard> {
  ...
  const builder = applyDefaults(
    new DashboardBuilder("Task Run").uid("taskrun"),
  )
        ...
        .withTarget(
          new DataqueryBuilder()
            ...
            WHERE
              ...
              AND last_payload -> 'subject' -> 'content' ->> 'taskName' = ANY(ARRAY[\${taskNames:sqlstring}]::text[])
              ORDER BY at DESC
              LIMIT $limit
          `),
        )
...
export function newVariable4Task() {
  return newVariableOnDatasource(
    dedent`
      SELECT DISTINCT payload -> 'subject' -> 'content' ->> 'taskName' AS taskName
      FROM cdevents_lake
