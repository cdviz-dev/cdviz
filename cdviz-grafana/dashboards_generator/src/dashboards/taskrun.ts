...
export async function buildDashboard(): Promise<Dashboard> {
  ...
  const builder = applyDefaults(new DashboardBuilder("Task Run").uid("taskrun"))
    .withVariable(newVariable4Task())
    .withVariable(new ConstantVariableBuilder("limit").value("20"))
    .withRow(new RowBuilder("Latest $limit Task Runs"))
    .withPanel(
      new D3PanelBuilder()
        .title("$task_names")
        .repeat("task_names")
        ...
        .withTarget(
          new DataqueryBuilder()
            ...
            WHERE
              ...
              AND last_payload -> 'subject' -> 'content' ->> 'taskName' = ANY(ARRAY[\${task_names:sqlstring}]::text[])
              ORDER BY at DESC
              LIMIT $limit
          `),
        )
...
export function newVariable4Task() {
  return newVariableOnDatasource(
    dedent`
      SELECT DISTINCT payload -> 'subject' -> 'content' ->> 'taskName' AS __value
      FROM cdevents_lake
