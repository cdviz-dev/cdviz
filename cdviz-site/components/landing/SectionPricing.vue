...
<template>
  <section class="my-xl md:my-2xl bg-gradient-to-b from-background to-secondary/5 py-16">
    ...
    <div class="text-xl text-center mb-8 max-w-3xl mx-auto">
      ...
      means core components are always free.
    </div>

    <!-- Pricing Toggle -->
    <div class="flex items-center justify-center mb-12">
      <span class="text-lg mr-4 font-medium" :class="{ 'text-current': isYearly, 'text-current/70': !isYearly }">Monthly</span>
      <button
      ...
      <span class="text-lg ml-4 flex items-center font-medium" :class="{ 'text-current': !isYearly, 'text-current/70': isYearly }">
        Annual
    ...
    <div class="grid gap-8 md:grid-cols-3 max-w-6xl mx-auto">
      ...
      >
        <div>
          ...
          <div class="text-4xl font-bold mx-auto my-4 text-primary">€{{ getPrice('community') }}</div>
      ...
      >
        <div class="absolute z-10 right-2 top-1 rotate-6 rounded-md px-2 py-0.5 text-center align-middle text-accent ring-accent ring-1 bg-accent/10 ring-offset-accent/20 ring-offset-1">preview</div>
        <div class="isDisabled">
          ...
          <ul class="my-md ml-2 text-left">
            ...
            <li class="check-circle mb-4 pl-8">Dashboard operated by you or your provider</li>
          </ul>
        </div>
    ...
    <div class="text-center mt-12 text-current/90">
      <p>
        All prices are in Euro (€) and exclude VAT. The Community edition is free forever, while the Enterprise and SaaS editions offer additional features and support. For more information or custom request, please <a href="/contact" class="text-primary hover:underline">contact us</a>.
      </p>
    </div>
  </section>
</template>
