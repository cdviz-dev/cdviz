    ...
    <div class="mx-auto max-w-5xl">
      <template v-for="aq in $frontmatter.faq" :key="aq.q">
        ...
        <div v-if="aq.a === ''" data-animate-faq class="my-xl first:mt-0">
          <h3 class="text-xl sm:text-2xl font-bold text-center text-primary">
            {{ aq.q }}
          </h3>
          <div
            class="w-12 sm:w-16 h-1 bg-primary mx-auto mt-xs rounded-full"
          ></div>
        </div>
        <!-- FAQ Item -->
        <details
        ...
        >
          <summary
            class="text-lg sm:text-xl font-semibold cursor-pointer list-none outline-none focus-ring rounded-lg p-xs -m-xs text-text"
          >
            <span class="flex items-center justify-between">
              <span>{{ aq.q }}</span>
              <span
                class="text-primary text-xl ml-sm transition-transform duration-300 ease-out group-open:rotate-180"
                >▼</span
              >
            </span>
          </summary>
          <div
            class="pt-md text-sm sm:text-base text-text/90 space-text leading-relaxed"
            v-html="aq.a"
          ></div>
        </details>
      </template>
