...
describe('MermaidConverter', () => {
  ...
  test('should apply CDviz styling', async () => {
    ...
    const styledElements = result.elements.filter(el => 
      el.strokeColor === '#f08c00' && el.roughness === 2
    );
  ...
  test('should process batch of files', async () => {
    ...
    console.log(`✅ Batch conversion: ${results.length} files processed, ${totalOutputs} outputs generated`);
  });
