...
export default makeScene2D(function* (view) {
  ...
  yield* sequence(
    ...
    all(
      subtitleRef().opacity(1, 0.9, easings.gentle),
      subtitleRef().y(-430, 0.9, easings.smooth),
    ),
  ...
  yield* all(
    ...
    ...connectionLines.map(line => all(
      line.opacity(0, 0.9, easings.gentle),
      line.lineWidth(2, 0.9, easings.smooth),
    )),
    ...questionMarks.map(mark => all(
      mark.opacity(0, 0.8, easings.smooth),
      mark.scale(0.7, 0.8, easings.gentle),
    )),
  );
  ...
  yield* all(
    ...
    ...eventParticles.map(particle => all(
      particle.opacity(0, 0.7, easings.smooth),
      particle.scale(0.3, 0.7, easings.gentle),
    )),
  );
  ...
  const features = [
    { icon: "/assets/icons_48x48_monochrome/mdi--graph-timeline-variant.svg", text: "Timeline View" },
  ...
  yield* sequence(
    ...
    ...featureLayouts.map((layout, i) =>
      all(
        ...
        layout.x(-20 + Math.sin(i * 0.5) * 10, 0).to(0, 0.9, easings.smooth), // Subtle entry motion
      )
    )
  );
  ...
  yield* all(
    ...featureLayouts.map((layout, i) => 
      all(
        layout.y(layout.y() + (i % 2 === 0 ? 5 : -5), 3.0, easings.gentle).to(layout.y(), 3.0, easings.gentle),
      )
    )
  );
  ...
  yield* all(
    ...
    ...featureLayouts.map((layout, i) => all(
  ...
  yield* all(
    ctaRef().opacity(1, 1.8, easings.organic),
    ctaRef().scale(1, 1.8, easings.bounce),
  );

  // Simplified breathing effect - single cycle, less intensive
  const mainText = ctaRef().children()[0]; // cdviz.dev text
  yield* all(
    mainText.scale(1.015, 2.0, easings.gentle).to(1, 2.0, easings.gentle),
  );
